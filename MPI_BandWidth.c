	...
	if (rank < numtasks / 2) {
		for (n = start; n <= end; n = n + incr) {
			...
			for (i = 1; i <= rndtrps; i++) {
				...
				MPI_Recv(&msgbuf, n, MPI_CHAR, src, tag, MPI_COMM_WORLD,
						&status);
				t2 = MPI_Wtime();
				thistime = t2 - t1;
			...
			if (rank == 0) {
				...
				for (j = 1; j < numtasks / 2; j++)
					MPI_Recv(&timings[j], 3, MPI_DOUBLE, j, tag, MPI_COMM_WORLD,
							&status);
				printf(
						"***Message size: %8d *** best  /  avg  / worst (MB/sec)\n",
						n);
				for (j = 0; j < numtasks / 2; j++) {
					printf(
							"   task pair: %4d - %4d:    %4.2f / %4.2f / %4.2f \n",
							j, taskpairs[j], timings[j][0], timings[j][1],
							timings[j][2]);
					bestall += timings[j][0];
					avgall += timings[j][1];
				...
				printf(
						"   OVERALL AVERAGES:          %4.2f / %4.2f / %4.2f \n\n",
