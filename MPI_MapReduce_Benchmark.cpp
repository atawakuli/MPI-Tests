...
long MESSAGE_SIZE = 524292, SENT = 0, RECEIVED = 0;
bool ACTIVE = false;
...
typedef struct pthread_p {
    ...
    long long int partialSumOfSquares; // the result of the computation
} pthread_p;
...
class MPI_MapReduce {
  ...
  protected:
    ...
    static const int windowSize = 1048576;
  ...
  public:
    ...
    MPI_MapReduce(int worldSize, int rank) {
      ...
      MESSAGE_SIZE = (windowSize + n)/n*sizeof(int); // caution: assumes uniform partitions!      
      ...
      if (rank == 0) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          partitions[i] = new int[windowSize];
        }
    ...
    static void* mpiReduceFunc(void* params) {
      ...
      long long int partialSumOfSquares = sum(partition, partitionSize);
    ...
    void* mpiMapFunc() {
      ...
      if (rank == 0) { // Master node
        ...
        long long int totalSumOfSquares = 0;
        for (int i = 0; i < n; i++) {
